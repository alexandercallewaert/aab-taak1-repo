...
## Generally speaking, same distribution; but also intermediate cases between 1000 and 15000
...
```{r}
library(corrplot)
corrplot(cor(train_ready2[,c(2,3,9)]), method = "circle")
## Nutteloos denk ik
```

## Tidying the data

We're now going to use the package tidyr to clean up the train data
...
compute_NA_matrix(train2) # claim vehicle id is 18.72142, geen idee hoe dit komt, ik verwijder deze variabele gewoon uit de data. Moest eigenlijk al mee verwijderd zijn toen ik de factoren met meer dan 53 levels verwijderde...
...
test2 <- delete_categrocial(test2)
```
...
```{r Split Data}
# First we split the train data in test and training! 
...
```{r}
## Random forest op de train set van de train data! 
...
## Error rate berekenen van voorspelde modellen!   

predicted1 <- predict(RFmodel1, train1_test)
tab1 <- table(train1_testLabel$fraud, predicted1)
err1 <- (1-sum(diag(tab1))/sum(tab1))
tab1
err1

predicted2 <- predict(RFmodel2, train2_test)
tab2 <- table(train2_testLabel$fraud, predicted2)
err2 <- (1-sum(diag(tab2))/sum(tab2))*100
tab2
err2 # dit model doet het iets beter dan vorig model
```


```{r}
predicted_test_1 <- predict(RFmodel1, newdata = test1, type = "prob")
predicted_test_1

probabilities1 <- predicted_test_1[,2]
claim_ids <- test_origineel[,1]
csv1 <- cbind(claim_ids, probabilities1)
head(csv1)
class(csv1)
write.csv(csv1, "CSV1.csv", row.names = FALSE)
```
...
table(train1_testLabel, predict(object = RFmodel1,newdata = train1_test))


```

Model toepassen op de echte testset, waar we geen labels van hebben en dus ook geen fouten van kunnen berekenen. 
...
head(train_ready2)
summary(train_ready2)
=======
load("data/test.RData")
predicted_test_1 <- data.frame(predict(RFmodel1, test[,], type="prob", predict.all=TRUE))
predicted_test_1

# Probleem is dat we NA values hebben voor bepaalde objecten, aangezien er nog missing values aanwezig zijn in de test set... 
```

>>>>>>> e61ba562f9c1f31b835f219e9507d58733ce069c
...
```{r}
pred.train <- predict(qda.out,books1[,5:6],CV=FALSE)
tab<-table(books1$buyer,pred.train$class)
print(tab)
