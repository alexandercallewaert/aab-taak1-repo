...
NA_variables_threshold <- function(dataset, threshold)
  {NA_list <- c()
...
```{r}
train <- train %>% select(all_of(NA_variables_threshold(train, 20)))
...
test <- test %>% select(all_of(all_variables[-1]))
